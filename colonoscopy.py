...
"")
        ...
        self.label_4.setGeometry(QtCore.QRect(340, 140, 811, 101))
        font = QtGui.QFont()
    ...
    def retranslateUi(self, Dialog):
        ...
        self.label_4.setText(_translate("Dialog", "             POLYP SEGMENTATION \n"
...
"    CENTER OF EXCELLENCE IN MEDICAL IMAGING              "))
        ...
        self.ab=None
        self.out=None
        self.segmodel=tf.keras.models.load_model(r"C:\CoE Research Works\project works\colonoscopy\polyp segmentation model-1.h5",compile=False)
        self.classmodel=tf.keras.models.load_model(r"C:\CoE Research Works\project works\colonoscopy\polyp classification model.h5")
    def loadimage(self):
    ...
    def predictimage(self):
        ...
        result = self.segmodel.predict(self.tmp)   
        result=(result>=0.5).astype(np.uint8)
        result1=np.squeeze(result)
        fig = plt.figure()
        fig.subplots_adjust(hspace=1, wspace=1)
        ax = fig.add_subplot(1, 1, 1)
        ax.imshow(np.reshape(result1, (256,256)), cmap="gray")
        plt.axis('off')
        ax.axes.get_xaxis().set_visible(False)
        ax.axes.get_yaxis().set_visible(False)
        save_path = r"C:\CoE Research Works\project works\colonoscopy\output2.png"
        ...
        ou=self.classmodel.predict(result)
        out=ou[0]
        if out<0.5:
            self.outcome.setText('Non-Polyp')
            self.about.setText('The segmented pathological region is observed to be \n non-cancerous and can be due to some inflammation.')
        else:
            self.outcome.setText('Polyp')
            self.about.setText('The segmented pathological region is observed to be cancerous.')
